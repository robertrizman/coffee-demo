...
export async function buildBrotherQLLabelsHtml(order, visitorId = '', useShorthand = false) {
  ...
  const parts = await Promise.all(items.map(async (item, index) => {
    const deepLink = buildQrDeepLink({ order, visitorId });
    const qrSvg = await buildQrSvg(deepLink, 200);
    const qrUrl = `data:image/svg+xml;base64,${btoa(qrSvg)}`;

    const customerName = order?.name || 'Guest';
    const orderId = esc(order.id || '');
    const placedTime = formatTime(order?.placedAt);
    const itemCount = items.length > 1 ? ` (${index + 1}/${items.length})` : '';

    const nameLines = wrapName(esc(customerName), 15);
    const specialReq = item.specialRequest ? esc(item.specialRequest) : '';

    let infoSvg = '';
    let dy = 14;

    if (items.length > 1) {
      infoSvg += `<text x="${QR_X + QR_W / 2}" y="35.5" font-size="2.8" font-weight="700" fill="#5a8888" font-family="Arial,Helvetica,sans-serif" text-anchor="middle">DRINK ${index + 1} OF ${items.length}</text>`;
    }

    if (useShorthand) {
      const shLines = toShorthandLines(item);
      shLines.forEach((line, i) => {
        const size = i === 0 ? 2.4 : 1.8;
        const color = i === 0 ? '#1a7a7a' : '#0d2b2b';
        infoSvg += `<text x="${INFO_X}" y="${dy}" font-size="${size}" font-weight="800" fill="${color}" font-family="Arial,Helvetica,sans-serif">${esc(line)}</text>`;
        dy += size * 1.45;
      });
    } else {
      if (item.name) {
        const drinkLines = wrapName(esc(item.name), 14);
        drinkLines.forEach(line => {
          infoSvg += `<text x="${INFO_X}" y="${dy}" font-size="3.5" font-weight="800" fill="#0d2b2b" font-family="Arial,Helvetica,sans-serif">${line}</text>`;
          dy += 4.8;
        });
      }
      const sizeMilk = [item.size, (item.milk && item.milk !== 'No Milk') ? item.milk : null].filter(Boolean);
      if (sizeMilk.length) {
        infoSvg += `<text x="${INFO_X}" y="${dy}" font-size="2.7" font-weight="700" fill="#5a8888" font-family="Arial,Helvetica,sans-serif">${esc(sizeMilk.join(' · '))}</text>`;
        dy += 3.5;
      }
      if (item.extras?.length) {
        infoSvg += `<text x="${INFO_X}" y="${dy}" font-size="3" fill="#5a8888" font-family="Arial,Helvetica,sans-serif">${esc(item.extras.join(', '))}</text>`;
        dy += 3.5;
      }
      if (specialReq) {
        const noteLines = wrapName('Notes: ' + specialReq, 25);
        noteLines.forEach(line => {
          infoSvg += `<text x="${INFO_X}" y="${dy}" font-size="2.5" fill="#5a8888" font-family="Arial,Helvetica,sans-serif" font-style="italic">${line}</text>`;
          dy += 3.2;
        });
      }
    }
    ...
    let bottomSvg = '';
    let bY = 38.5 - (nameLines.length - 1) * 4.2;
    nameLines.forEach(line => {
      bottomSvg += `<text x="1.5" y="${bY}" font-size="4.5" font-weight="900" fill="#0d2b2b" font-family="Arial,Helvetica,sans-serif">${line}</text>`;
      bY += 4.2;
    });
  ...
  <g transform="matrix(0,-1,1,0,0,48)">
    ...
    <rect x="0.3" y="6" width="47.4" height="1.3" fill="#1a7a7a"/>
    <image href="data:image/png;base64," x="30" y="0.8" width="17.7" height="6" preserveAspectRatio="xMaxYMid meet"/>
    <text x="1.5" y="5" font-size="4.2" font-weight="800" fill="white" font-family="Arial,Helvetica,sans-serif" text-anchor="start">${orderId}${itemCount}</text>
    <text x="${QR_X + QR_W / 2}" y="9.5" font-size="2.8" fill="#0d2b2b" font-family="Arial,Helvetica,sans-serif" text-anchor="middle">Printed: ${placedTime}</text>
    ${infoSvg}
    <image href="${qrUrl}" x="${QR_X}" y="11" width="${QR_W}" height="${QR_H}"/>
    <text x="${QR_X + QR_W / 2}" y="31.5" font-size="2.2" fill="#aaaaaa" font-family="Arial,Helvetica,sans-serif" text-anchor="middle">SCAN WHEN READY</text>
    <line x1="0.5" y1="32" x2="47.5" y2="32" stroke="#e0e0e0" stroke-width="0.3"/>
    ${bottomSvg}
  </g>
</svg></div>`;
  }));
  ...
  <style>
  @page { size: 40mm 48mm; margin: 0; }
  html, body { margin: 0; padding: 0; background: #fff; }
  .sheet { width: 40mm; height: 48mm; page-break-after: always; break-after: page; }
  ...
  if (isBrotherPrinterAvailable()) {
    ...
    if (connectionType === 'bluetooth' && printer.bluetoothAddress) {
      ...
      try {
        ...
        if (err?.code === 'CHANNEL_ERROR') {
          console.log('[Printer] BT stream stale, retrying in 2s...');
