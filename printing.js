...
export async function buildBrotherQLLabelsHtml(order, visitorId = '', useShorthand = false) {
  ...
  const parts = await Promise.all(items.map(async (item, index) => {
    ...
    if (useShorthand) {
      ...
      infoContent = shLines.map((line) => `<div class="sh-line">${esc(line)}</div>`).join('');
    } else {
      const nameLine    = item.name ? `<div class="info-row info-drink">${esc(item.name)}</div>` : '';
      ...
      <div class="sheet">
        <div class="label">
          <div class="top">
            ...
            <div class="top-right">
              <div class="order-id">${orderId}${itemCount}</div>
              <div class="order-time">${placedTime}</div>
            </div>
          </div>
          <div class="content">
  ...
  * { box-sizing: border-box; }
  ...
  .order-id { font-size: 3mm; font-weight: 800; color: #fff; }
  .order-time { font-size: 1.8mm; color: rgba(255,255,255,0.85); }
  .content { flex: 1; padding: 0.8mm 1.5mm 0.5mm; display: flex; flex-direction: column; overflow: hidden; }
  .qr-wrap { display: flex; flex-direction: column; align-items: center; margin-bottom: 1mm; flex-shrink: 0; }
  .qr { width: 20mm; height: 20mm; border: 0.2mm solid #ddd; padding: 0.5mm; background: #fff; }
  .qr svg { width: 100%; height: 100%; display: block; }
  .scan { font-size: 1.4mm; color: #8a8a8a; margin-top: 0.4mm; letter-spacing: 0.3mm; text-transform: uppercase; text-align: center; }
  .customer { font-size: 3.4mm; font-weight: 900; color: #0d2b2b; line-height: 1.1; margin-bottom: 0.5mm; word-break: break-word; }
